...
import { createEconomy, createPlayerModels, ENGINE_VERSION, generateDraftClass } from './simulationData'
...
function migrateFoundation(value: Record<string, unknown>): CareerV2 | null { const teamAbbr = String(value.teamAbbr ?? ''); const team = teams.find((candidate) => candidate.abbr === teamAbbr); if (!team) return null; const base = makeCareer(team, (value.manager as CareerV2['manager']) ?? { name: 'Manager Vale', origin: 'Brasil', experience: 'Carreira Foundation', style: 'Equilibrado', avatar: 'manager-01.png', reputation: 50 }, (value.language as Language) ?? 'pt-BR'); const migrated = { ...base, ...value, version: 3 as const, schemaVersion: 3 as const, engineVersion: ENGINE_VERSION, playerModels: createPlayerModels(players), economy: createEconomy(players, teams), tradeDesk: [], playoffs: [], seasonArchives: [], seasonNumber: 1, simulationLog: [`${new Date().toISOString()}: save Foundation v2 migrado para Simulation v3.`], generatedPlayers: [], retiredPlayerIds: [], draftHistory: [] } as CareerV2; return migrated }
function completeV3(value: CareerV2): CareerV2 { return { ...value, generatedPlayers: value.generatedPlayers ?? [], retiredPlayerIds: value.retiredPlayerIds ?? [], draftHistory: value.draftHistory ?? [] } }
export function loadCareer(): CareerV2 | null { try { const v3 = JSON.parse(localStorage.getItem(SAVE_KEY) || 'null'); if (v3?.version === 3) return completeV3(v3); const foundation = JSON.parse(localStorage.getItem(FOUNDATION_KEY) || 'null'); if (foundation?.version === 2) { const migrated = migrateFoundation(foundation); if (migrated) { localStorage.setItem(SAVE_KEY, JSON.stringify(migrated)); return migrated } } const legacy = JSON.parse(localStorage.getItem(LEGACY_KEY) || 'null') as Legacy | null; if (legacy?.version === 1) { const migrated = migrateLegacy(legacy); if (migrated) { localStorage.setItem(SAVE_KEY, JSON.stringify(migrated)); return migrated } } } catch { /* storage unavailable or malformed */ } return null }
