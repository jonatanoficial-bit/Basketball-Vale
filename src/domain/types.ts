...
export type GameEvent = {
  ...
  type: 'made2' | 'made3' | 'miss2' | 'miss3' | 'ft' | 'rebound' | 'turnover' | 'steal' | 'block' | 'foul' | 'timeout' | 'substitution' | 'injury' | 'period'
...
export type GameResult = {
  ...
  seed?: string; engineVersion?: string; overtime?: number; advanced?: Record<string, AdvancedTeamStats>
  lineups?: LineupStat[]; shotChart?: ShotChartPoint[]; injuries?: { playerId: number; diagnosis: string; recoveryGames: number }[]; checksum?: string
}
...
export type PlayerModel = {
  playerId: number; age: number; archetype: string; roles: string[]; hiddenPotential: number
  offense: number; shooting: number; finishing: number; playmaking: number; defense: number
  rebounding: number; athleticism: number; iq: number; durability: number
  tendencies: { three: number; rim: number; pass: number; turnover: number; foulDraw: number }
  morale: number; readiness: number; personality: 'Líder' | 'Competidor' | 'Reservado' | 'Mentor'
  phase: 'development' | 'prime' | 'plateau' | 'regression'
}

export type SimContract = {
  playerId: number; team: string; salary: number; years: number; startSeason: string
  option?: 'team' | 'player'; status: 'active' | 'free-agent' | 'waived'; source: 'SIMULATION'
}

export type LeagueEconomy = {
  cap: number; tax: number; firstApron: number; secondApron: number; rulesVersion: string
  contracts: Record<number, SimContract>; capHolds: Record<string, number>; exceptions: Record<string, number>
}

export type AdvancedTeamStats = {
  possessions: number; offensiveRating: number; defensiveRating: number; pace: number
  effectiveFg: number; trueShooting: number; turnoverRate: number; offensiveReboundRate: number; freeThrowRate: number
}
export type LineupStat = { team: string; playerIds: number[]; minutes: number; pointsFor: number; pointsAgainst: number; netRating: number }
export type ShotChartPoint = { team: string; playerId: number; x: number; y: number; made: boolean; value: 2 | 3; period: number }
export type TradeAsset = { kind: 'player' | 'pick'; id: string; team: string }
export type TradeProposal = { id: string; teams: string[]; outgoing: TradeAsset[]; incoming: TradeAsset[]; score: number; valid: boolean; reasons: string[] }
export type PlayoffSeries = { id: string; conference: string; round: number; higherSeed: string; lowerSeed: string; higherWins: number; lowerWins: number; winner?: string }
export type SeasonArchive = { season: string; champion?: string; standings: TeamRecord[]; awards: Record<string, string>; draftClassId?: string }
export type DraftHistoryEntry = { season: number; pick: number; team: string; playerId: number; name: string; position: string }

export type CareerV2 = {
  version: 3; schemaVersion: 3; engineVersion: string; createdAt: string; currentDate: string; teamAbbr: string; language: Language; manager: ManagerProfile
  ...
  playerModels: Record<number, PlayerModel>; economy: LeagueEconomy; tradeDesk: TradeProposal[]
  playoffs: PlayoffSeries[]; seasonArchives: SeasonArchive[]; seasonNumber: number; simulationLog: string[]
  generatedPlayers: Player[]; retiredPlayerIds: number[]; draftHistory: DraftHistoryEntry[]
}
