...
export function simulateRounds(career: CareerV2, players: Player[], roundCount: number) {
  const rounds = [...new Set(career.fixtures.filter((fixture) => fixture.status === 'scheduled').map((fixture) => fixture.round))].sort((a,b) => a-b).slice(0, roundCount)
  const activePlayers = universePlayers(career, players); const games = career.fixtures.filter((fixture) => fixture.status === 'scheduled' && rounds.includes(fixture.round)).map((fixture) => simulateFixture(fixture, activePlayers, career.teamAbbr, career.rotation, career.tactics, false, career.playerModels, career.health)); const next = applyResults(career, games)
  next.news.unshift({ id: `fast-${Date.now()}`, date: next.currentDate, category: 'league', title: `${rounds.length} rodada(s) simulada(s)`, body: `${games.length} partidas foram processadas pela mesma engine determinística, sem renderização.` })
  return next
}

function playSeries(career: CareerV2, series: PlayoffSeries, players: Player[], round: number): PlayoffSeries {
  const activePlayers = universePlayers(career, players); let higherWins = 0; let lowerWins = 0; let gameIndex = 0
...
const makeRookie = (prospect: Prospect, season: number, pick: number, team: string): Player => {
  ...
  const [firstName, ...rest] = prospect.name.split(' '); const lastName = rest.join(' ')
  return { id, firstName, lastName, name: prospect.name, slug: `dev-${season}-${pick}`, teamId: 0, team, jersey: String((pick * 3) % 55), position: prospect.position, height: prospect.position === 'C' ? '6-11' : prospect.position === 'PG' ? '6-3' : '6-7', weight: prospect.position === 'C' ? '245 lbs' : '210 lbs', college: 'SIMULATION Draft Class', country: 'DEV', overall: Math.max(64, 82 - Math.floor(pick / 2)), headshot: 'assets/nba/players/generated.svg' }
}

/** Complete offseason: options/expiries, deterministic retirement, 30-pick draft and AI roster balancing. */
export function advanceOffseason(career: CareerV2, players: Player[], teams: Team[]) {
  const next = structuredClone(career); const endingSeason = next.seasonNumber
  Object.values(next.economy.contracts).forEach((contract) => { if (contract.status !== 'active') return; contract.years -= 1; if (contract.years <= 0) { contract.status = 'free-agent'; contract.team = 'FA'; contract.years = 0 } })
  next.playerModels = developModels(next.playerModels, endingSeason)
  for (const model of Object.values(next.playerModels)) { const random = randomFromSeed(`retire-${endingSeason}-${model.playerId}`); if (model.age >= 38 && random() < Math.min(.92, .24 + (model.age - 38) * .17)) { next.retiredPlayerIds.push(model.playerId); const contract = next.economy.contracts[model.playerId]; if (contract) { contract.status = 'waived'; contract.team = 'RET'; contract.years = 0 } } }
  const order = Object.values(next.standings).sort((a,b) => a.wins - b.wins || a.pf - a.pa).map((row) => row.team)
  const draftClass = next.prospects.length >= 30 ? next.prospects : generateDraftClass(2026 + endingSeason)
